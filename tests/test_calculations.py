...
def test_get_latest(setup_calculations):
    ...
    assert latest.a == Decimal('20') and latest.b == Decimal('3'), "No correct latest calculation"
...
def test_find_by_operation(setup_calculations):
    ...
    assert len(add_operations) == 1, "Did not find the correct number of calculations add"
    ...
    assert len(subtract_operations) == 1, "Didn't find correct number of calculations subtract"
...
def test_get_latest_with_empty_history():
    ...
    assert Calculations.get_latest() is None, "Expected None latest having empty history"
